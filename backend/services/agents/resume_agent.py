...
def parse_resume(raw_text: str, filename: Optional[str] = None,
    ...
    This is a pure function with no database dependency. Caching (if needed)
    should be handled by the caller.

    Args:
        raw_text: Raw extracted text from the resume file.
        filename: Optional original filename (used for name extraction fallback).
        config: Optional GeminiConfig. If None or missing API key, uses local fallback.

    Returns:
        Normalized dict with 30+ structured fields (candidate_name, skills, etc.).
    """
    if config is None:
...
# ---------------------------------------------------------------------------
# Mock Data (for dev/test without API key)
# ---------------------------------------------------------------------------
