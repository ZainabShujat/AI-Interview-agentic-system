...
import models
import schemas
from services import gemini_service, pdf_service, speech_service
...
async def submit_audio_answer(
    ...
    try:
        ...
        answer_text = transcription_result.get("transcript", "")
        
        # Analyze acoustic features
        acoustic_features = speech_service.analyze_voice_features(audio_bytes)
        
        # Parse signals and combine with acoustic features
        try:
            signals_dict = json.loads(signals)
        except json.JSONDecodeError:
            signals_dict = {}
        signals_dict.update(acoustic_features)
        ...
        db_answer = models.Answer(
            ...
            evaluation_json=evaluation,
            confidence_metrics=signals_dict
        )
        db.add(db_answer)
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error processing audio response: {str(e)}")
...
async def get_report(interview_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        WorkflowOrchestrator.process_interview_completion(db, interview_id)
